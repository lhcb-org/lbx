...
func NewHelper(pkguri, pkgname, pkgid, pkgdir string) (*Helper, error) {
	...
	tmpdir, err := ioutil.TempDir("", "lbx-pkg-co-")
	if err != nil {
		return nil, err
	}
	...
	case "svn", "svn+ssh":
		...
		err = ioutil.WriteFile(filepath.Join(tmpdir, "version.lbx"), []byte(rev+"\n"), 0666)
		if err != nil {
			return nil, err
		}
...
func (h *Helper) git_checkout() error {
	...
	err = ioutil.WriteFile(filepath.Join(h.RepoDir, "version.lbx"), []byte(rev), 0666)
	if err != nil {
		return err
	}
